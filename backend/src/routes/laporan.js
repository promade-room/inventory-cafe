...
router.get('/stok', auth, async (req, res) => {
  try {
    const { periode } = req.query; // format: 2026-01
    
    let query = `
      SELECT 
        b.id, b.kode, b.nama, b.satuan, b.minimal_stok,
        k.nama as kategori_nama,
        COALESCE((SELECT SUM(bm.jumlah) FROM barang_masuks bm WHERE bm.barang_id = b.id), 0) as total_masuk,
        COALESCE((SELECT SUM(bk.jumlah) FROM barang_keluars bk WHERE bk.barang_id = b.id), 0) as total_keluar,
